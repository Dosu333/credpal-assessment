...
export class WalletService {
  ...
  async fundWallet(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!currency)
        throw new BadRequestException(`Invalid currency: ${currencyCode}`);
      ...
      const externalWallet = await this.getSystemWallet(
        manager,
        EXTERNAL_PROVIDER_ID,
      );
  ...
  async tradeCurrency(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const tx = manager.create(Transaction, {
        ...
        status: TransactionStatus.SUCCESS,
        metadata: { rate, convertedAmount, toCurrency },
      });
      await manager.save(tx);

      // Fetch Entities
      const fromCurrencyEntity = await this.currencyRepo.findOne({
        where: { code: fromCurrency },
      });
      const toCurrencyEntity = await this.currencyRepo.findOne({
        where: { code: toCurrency },
      });

      if (!fromCurrencyEntity)
        throw new BadRequestException(`Invalid currency: ${fromCurrency}`);
      if (!toCurrencyEntity)
        throw new BadRequestException(`Invalid currency: ${toCurrency}`);
  ...
  private async recordLedgerEntry(
    ...
    if (params.type === LedgerEntryType.CREDIT) {
      ...
      if (
        ...
        params.walletId !== SYSTEM_WALLET_ID &&
        params.walletId !== EXTERNAL_PROVIDER_ID
      ) {
